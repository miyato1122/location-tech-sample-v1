...
MapLibre GL のフルページマップを主UIとするシングルページアプリケーション（SPA）。マップのソース・レイヤー・インタラクションはすべて `main.js` 一本に定義されている。並行して Nuxt 3 への移行プロジェクト（`nuxt-map-app/`）が進行中。
...
- **`nuxt-map-app/`**: Nuxt 3 移行先。現在はスキャフォールドのみ（`node_modules` のみ、ソースファイルなし）
